...
// ── Wallpapers ────────────────────────────────────────────────────────────────
// Each wallpaper is now a real photo (Unsplash CDN). `bgUrl` is the full-size
// image used as the room backdrop; `thumbUrl` is the small version shown in
// the picker grid. Older `id`s like 'galaxy' are kept so existing rooms that
// already have a wallpaper saved on their RoomSession document keep working.
const _wpUrl       = (id, w, q) => `https://images.unsplash.com/photo-${id}?w=${w}&q=${q}&fit=crop&auto=format`;
const _wpStyle     = (id) => ({ backgroundImage: `url(${_wpUrl(id, 1280, 80)})`, backgroundSize: 'cover', backgroundPosition: 'center' });
const _wpThumb     = (id) => ({ backgroundImage: `url(${_wpUrl(id, 220,  60)})`, backgroundSize: 'cover', backgroundPosition: 'center' });

const WALLPAPERS = [
  { id: 'none',     label: 'None',     style: { background: '#0a0a0f' }, thumbStyle: { background: '#0a0a0f' } },
  { id: 'galaxy',   label: 'Galaxy',   photo: '1462331940025-496dfbfc7564' },
  { id: 'sunset',   label: 'Sunset',   photo: '1495616811223-4d98c6e9c869' },
  { id: 'ocean',    label: 'Ocean',    photo: '1505142468610-359e7d316be0' },
  { id: 'forest',   label: 'Forest',   photo: '1448375240586-882707db888b' },
  { id: 'midnight', label: 'Midnight', photo: '1419242902214-272b3f66ee7a' },
  { id: 'aurora',   label: 'Aurora',   photo: '1531366936337-7c912a4589a7' },
  { id: 'rose',     label: 'Rose',     photo: '1518895949257-7621c3c786d7' },
  { id: 'fire',     label: 'Fire',     photo: '1497032628192-86f99bcd76bc' },
  { id: 'purple',   label: 'Purple',   photo: '1493514789931-586cb221d7a7' },
  { id: 'dark_red', label: 'Dark Red', photo: '1547036967-23d11aacaee0' },
  { id: 'space',    label: 'Space',    photo: '1543722530-d2c3201371e7' },
].map(wp => wp.photo
  ? { ...wp, style: _wpStyle(wp.photo), thumbStyle: _wpThumb(wp.photo) }
  : wp);
      ...
      <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
        {WALLPAPERS.map(wp => (
          <button key={wp.id} onClick={() => { onSelect(wp.id); onClose(); }}
            className={`relative aspect-video rounded-lg overflow-hidden border-2 transition-all hover:scale-105 bg-dark-700 ${current === wp.id ? 'border-brand-400' : 'border-transparent'}`}
            style={wp.thumbStyle || wp.style}
            title={wp.label}>
            <span className="absolute inset-0 bg-black/40 flex items-center justify-center text-white text-[9px] font-bold drop-shadow">{wp.label}</span>
