...
const BG_PRESETS = [
  ...
  { id: 'aurora', label: 'Aurora', style: { background: 'linear-gradient(180deg,#00c9ff,#92fe9d)' } },
  { id: 'neon',   label: 'Neon',   style: { background: 'linear-gradient(135deg,#f72585,#4361ee)' } },
  { id: 'gold',   label: 'Gold',   style: { background: 'linear-gradient(135deg,#f9d423,#f83600)' } },
];
...
// ── Main Panel ────────────────────────────────────────────────────────────────
export default function LiveModerationPanel({ streamId, isHost, isAdmin: myIsAdmin }) {
  const { user }  = useAuth();
  ...
  const playSound = async (sound) => {
    ...
    } catch { toast.error('Failed to play sound'); }
  };
                    ...
                    <div className="grid grid-cols-3 gap-2">
                      {BG_PRESETS.map(bg => (
                        <button key={bg.id} onClick={() => applyBackground(bg.id)}
                          className={`relative aspect-video rounded-lg overflow-hidden border-2 transition-all hover:scale-105 ${currentBg === bg.id ? 'border-brand-400' : 'border-transparent'}`}
                          style={bg.style}>
                          <span className="absolute inset-0 bg-black/30 flex items-center justify-center text-white text-[9px] font-bold">{bg.label}</span>
                        </button>
                      ))}
                    </div>
                  </div>
