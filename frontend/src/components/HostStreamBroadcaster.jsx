...
import { ICE_SERVERS, preferH264Video } from '../utils/webrtcConfig';
...
export default function HostStreamBroadcaster() {
  ...
  useEffect(() => {
    ...
    const createPeer = async (viewerSocketId) => {
      ...
      // Use addTransceiver with explicit `sendonly` direction.
      //
      // The default `addTrack` path creates a `sendrecv` transceiver — i.e.
      // the SDP says "I want to send media AND receive media from you." iOS
      // Safari is much stricter about this than desktop browsers; when the
      // viewer never sends anything back (because there's nothing for them
      // to send) iOS can stall the negotiation and never fire `ontrack`.
      // Saying `sendonly` upfront tells iOS we don't expect return media,
      // which makes the offer/answer cycle complete reliably and ontrack
      // fires immediately on setRemoteDescription.
      tracks.forEach(t => {
        try {
          pc.addTransceiver(t, { direction: 'sendonly', streams: [localStream] });
        } catch (e1) {
          // Older browsers don't accept `streams` in init — fall back.
          try { pc.addTransceiver(t, { direction: 'sendonly' }); }
          catch (e2) {
            // Final fallback to addTrack so we don't fail entirely.
            try { pc.addTrack(t, localStream); }
            catch (e3) { warn('all track-add paths failed for', t.kind, e3); }
          }
        }
      });
