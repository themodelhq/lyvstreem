...
import {
  ...
  FiLogIn, FiMenu, FiX, FiBell, FiSearch, FiDollarSign,
  ...
  return (
    <div className="min-h-screen bg-dark-900 flex flex-col">
          ...
          <div className="flex items-center gap-2">
            {user ? (
              <>
                ...
                <Link to={`/profile/${user.username}`} className="w-8 h-8 rounded-full bg-gradient-to-br from-brand-500 to-brand-700 flex items-center justify-center text-sm font-bold overflow-hidden">
                  {user.avatar ? <img src={user.avatar} alt="" className="w-full h-full object-cover" /> : (user.displayName?.[0] || user.username?.[0] || 'U').toUpperCase()}
                </Link>
              </>
            ) : (
              <>
      ...
      <div className="flex flex-1">
        {/* Sidebar */}
        <aside className={`fixed md:sticky top-16 h-[calc(100vh-4rem)] w-56 bg-dark-800/50 border-r border-white/5 flex-col py-4 z-40 transition-transform duration-300 ${mobileOpen ? 'flex translate-x-0' : '-translate-x-full md:flex md:translate-x-0'}`}>
          <nav className="flex-1 px-3 space-y-1">
            {NAV_ITEMS.map(({ path, icon: Icon, label }) => (
              <Link key={path} to={path} onClick={() => setMobileOpen(false)}
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive(path) ? 'bg-brand-500/20 text-brand-400 border border-brand-500/30' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
            ...
            {user && (
              <>
                ...
                <Link to="/go-live" onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive('/go-live') ? 'bg-brand-500/20 text-brand-400 border border-brand-500/30' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
                  <BsCameraVideoFill className="text-lg" /> Go Live
                </Link>

                <Link to="/host-dashboard" onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive('/host-dashboard') ? 'bg-brand-500/20 text-brand-400 border border-brand-500/30' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
                  <FiBarChart2 className="text-lg" /> Host Dashboard
                </Link>

                <Link to="/withdraw" onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive('/withdraw') ? 'bg-brand-500/20 text-brand-400 border border-brand-500/30' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
                  <FiDollarSign className="text-lg" /> Withdraw
                </Link>
                ...
                <Link to="/coins" onClick={() => setMobileOpen(false)}
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all ${isActive('/coins') ? 'bg-brand-500/20 text-brand-400 border border-brand-500/30' : 'text-white/60 hover:text-white hover:bg-white/5'}`}>
          ...
          <div className="px-3 space-y-1 border-t border-white/5 pt-3">
            {user ? (
              <>
        ...
        {mobileOpen && <div className="md:hidden fixed inset-0 bg-black/50 z-30 top-16" onClick={() => setMobileOpen(false)} />}
