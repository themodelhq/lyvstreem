...
import { FiTrendingUp, FiZap, FiRefreshCw } from 'react-icons/fi';
  ...
  const fetchStreams = async (quiet = false) => {
    ...
    try {
      const res = await api.get('/streams/live', {
        params: { category: activeCategory === 'All' ? undefined : activeCategory, limit: 24 },
      });
    ...
    <div className="p-4 md:p-6 max-w-screen-2xl mx-auto">
        ...
        <div className="relative rounded-2xl overflow-hidden mb-8 bg-gradient-to-br from-brand-900 via-dark-700 to-dark-900 border border-brand-500/20 p-8">
          <div className="relative z-10">
            ...
            <p className="text-white/60 mt-3 text-lg max-w-lg">
              Watch live streams, send gifts, chat in real-time. Join creators and viewers now.
            </p>
      ...
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-white font-display font-bold text-xl flex items-center gap-2">
          ...
          Live Now
          {streams.length > 0 && (
            <span className="text-brand-500 text-sm font-normal">{streams.length} streams</span>
          )}
        </h2>
        <div className="flex items-center gap-2">
          ...
          <Link to="/discover" className="text-brand-400 hover:text-brand-300 text-sm transition-colors">
            See all →
          </Link>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-20">
          ...
          <p className="text-white/60 text-lg font-medium">No live streams right now</p>
          <p className="text-white/30 text-sm mt-2">Check back soon or be the first to go live!</p>
          {user && (
            <Link to="/go-live" className="btn-primary mt-6 inline-flex items-center gap-2">
              <BsCameraVideoFill /> Go Live Now
            </Link>
          )}
        </div>
      ) : (
      ...
      {user && !loading && (
        <div className="mt-12 text-center py-8 border border-dashed border-white/10 rounded-2xl">
          ...
          <p className="text-white/50 text-sm mt-1">Share your world with viewers</p>
