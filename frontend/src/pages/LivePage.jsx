...
export default function LivePage() {
  ...
  // Bind the received MediaStream to the visible <video> element. The video
  // is always rendered (hidden until hostStream arrives) so the ref is
  // attached well before the stream comes in. We always start muted in the
  // `muted` attribute — that's the one autoplay mode every browser allows —
  // and let the user opt into audio with a single tap.
  useEffect(() => {
    ...
    if (hostStream && v.srcObject !== hostStream) {
      ...
      if (p && typeof p.catch === 'function') {
        p.catch((e) => console.warn('[lyvstream/viewer] play() rejected:', e));
      }
