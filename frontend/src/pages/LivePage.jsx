...
export default function LivePage() {
  ...
  // Visible diagnostic — replaces "Connecting to live video…" with the
  // actual WebRTC state. Lets iOS users see where things are stuck without
  // needing Safari Web Inspector / a Mac.
  ...
  useEffect(() => {
    ...
    let retryTimer = null;
    // Count consecutive `solo_host_offline` events. A single one is almost
    // always a transient socket reconnect on the host (mobile network blip,
    // free-tier server hiccup, etc.); only escalate the user-visible status
    // to "Host is offline" after sustained failure.
    let consecutiveOfflines = 0;
    ...
    const ensurePeer = () => {
      ...
      pc.ontrack = (e) => {
        ...
        setConnectStatus(`Receiving ${e.track.kind}…`);
      ...
      pc.oniceconnectionstatechange = () => {
        ...
        if (!cancelled) setConnectStatus(`ICE: ${pc.iceConnectionState}`);
      };
      pc.onconnectionstatechange = () => {
        log('connection state:', pc.connectionState);
        if (!cancelled) setConnectStatus(`Connection: ${pc.connectionState}`);
        if (['failed', 'closed'].includes(pc.connectionState)) {
          if (!cancelled) {
            setHostStream(null);
            assembledStream = null;
            remoteSet = false;
            // Close the dead PC so the next solo_offer creates a fresh one.
            try { pc.close(); } catch (_) {}
            pc = null;
            setConnectStatus('Reconnecting to host…');
    ...
    const onOffer = async ({ fromSocketId, offer }) => {
      ...
      // We just got a real offer — clear the transient-offline counter.
      consecutiveOfflines = 0;
      setConnectStatus('Negotiating…');
      hostSocketId = fromSocketId;
      const peer = ensurePeer();
      if (!peer || cancelled) return;
      try {
        // Wrap in RTCSessionDescription — older iOS Safari builds reject
        // plain objects here even though desktop browsers accept them.
        ...
        setConnectStatus('Answer sent, waiting for media…');
        socket.emit('solo_answer', { targetSocketId: fromSocketId, answer, streamId });
      } catch (e) {
        warn('handling offer failed:', e);
        setConnectStatus(`Negotiation failed: ${e?.name || 'error'}`);
      }
    };
    ...
    const onHostOffline = () => {
      ...
      warn('host_offline received (', consecutiveOfflines, 'consecutive)');
      // Show transient "Reconnecting…" for the first few — a single offline
      // event almost always corresponds to a brief socket reconnect on the
      // host. Only escalate to the alarming "Host is offline" message after
      // sustained failure (~12 s of no contact).
      if (consecutiveOfflines >= 4) {
        setHostStream(null);
        setConnectStatus('Host is offline');
      } else {
        setConnectStatus('Reconnecting to host…');
      }
    };
    ...
    const requestStream = () => {
      ...
      // Only set the "Requesting…" text on the very first attempt. After
      // that, leave the status alone so the offline counter / negotiation
      // logic owns it — otherwise the text flips back and forth between
      // "Requesting stream from host…" and "Reconnecting to host…" every
      // 3 s when the host's socket is briefly unreachable.
      if (firstRequest) {
        setConnectStatus('Requesting stream from host…');
        firstRequest = false;
      }
      socket.emit('solo_request_stream', { streamId });
