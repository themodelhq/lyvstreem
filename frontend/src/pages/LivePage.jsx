...
export default function LivePage() {
  ...
  useEffect(() => {
    ...
    const ensurePeer = () => {
      ...
      pc.ontrack = (e) => {
        ...
        // Prefer the stream the host attached, fall back to building one
        // from the individual track when streams[0] is empty.
        const fromHost = e.streams && e.streams[0];
        if (fromHost) {
          assembledStream = fromHost;
        } else {
  ...
  //
  // Mobile (especially iOS Safari) needs special handling:
  //   • `muted`, `playsInline`, `autoplay` MUST be set on the DOM element
  //     directly before assigning srcObject. React's `muted` prop is known
  //     to occasionally not stick on iOS.
  //   • Some legacy iOS / Tencent X5 browsers want `webkit-playsinline` and
  //     `x5-playsinline` attributes.
  //   • play() often rejects on first call even when muted; we retry on
  //     `loadedmetadata` and `canplay` events. If it still rejects, we
  //     surface a full-screen Tap-to-play button so the user can recover.
  useEffect(() => {
    ...
    // Set every relevant property/attribute IMPERATIVELY before srcObject.
    v.muted = muted;
    v.playsInline = true;
    v.autoplay = true;
    v.controls = false;
    ...
    try { v.setAttribute('x5-playsinline', ''); }    catch (_) {}
    ...
    // Try immediately
    tryPlay();
    // …and again on the events that signal the video is ready to play. iOS
    // often needs the second attempt after `loadedmetadata`.
    ...
    <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)] bg-black overflow-hidden relative">
      ...
      <div className="relative flex-1 bg-black flex flex-col min-h-0" style={streamBackground.startsWith('http') || streamBackground.startsWith('/') ? { backgroundImage: `url(${streamBackground})`, backgroundSize: 'cover', backgroundPosition: 'center' } : streamBackground ? { background: (() => { const g = { galaxy:'linear-gradient(135deg,#0f0c29,#302b63,#24243e)', sunset:'linear-gradient(135deg,#f83600,#f9d423)', ocean:'linear-gradient(135deg,#1a6dff,#0ad3ff)', rose:'linear-gradient(135deg,#f953c6,#b91d73)', forest:'linear-gradient(135deg,#134e5e,#71b280)', fire:'linear-gradient(135deg,#f12711,#f5af19)', purple:'linear-gradient(135deg,#6a0572,#c850c0)', space:'radial-gradient(ellipse at center,#1b2735 0%,#090a0f 100%)', aurora:'linear-gradient(180deg,#00c9ff,#92fe9d)', neon:'linear-gradient(135deg,#f72585,#4361ee)', gold:'linear-gradient(135deg,#f9d423,#f83600)' }; return g[streamBackground] || ''; })() } : {}}>
        ...
        <div className="flex-1 relative min-h-0">
          ...
          {!isRoomMode && (
            <>
              {/* Live video from the host. ALWAYS rendered (hidden until a
                  remote stream arrives) so the ref is attached well before
                  the WebRTC connection establishes. The `muted` attribute
                  is what allows autoplay to fire on every browser; the user
                  opts into audio with a single tap. */}
              <video
                ref={hostVideoRef}
                autoPlay playsInline
                muted={muted}
                onClick={muted ? enableSound : undefined}
                // The hard-coded HTML attributes below are belt-and-braces
                // for older iOS / Android in-app browsers that don't honour
                // the React props.
                webkit-playsinline=""
                x5-playsinline=""
                className={`absolute inset-0 w-full h-full object-cover bg-black z-10 transition-opacity duration-300 ${hostStream ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
              />
              ...
              {/* Placeholder — only when there's no remote video yet, so its
                  backdrop-blur cannot ever clobber the video. */}
              {!hostStream && (
