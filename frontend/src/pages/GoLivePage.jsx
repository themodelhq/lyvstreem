...
import {
  ...
  FiCrosshair, FiShare2, FiDollarSign, FiClock, FiMinimize2, FiRotateCcw
} from 'react-icons/fi';
...
export default function GoLivePage() {
  ...
  useEffect(() => {
    ...
    if (restoreId && liveCtx?.activeStream?._id === restoreId) {
      ...
      setLocalStream(liveCtx.localStream?.current);
  ...
  const startCamera = async (facing = facingMode) => {
    try {
      ...
      const constraints = {
        video: isMobile() ? { facingMode: facing } : true,
        audio: true,
      };
      const s = await navigator.mediaDevices.getUserMedia(constraints);
      setLocalStream(s);
      if (liveCtx) liveCtx.localStream.current = s;
      setCamError(false);
  ...
  const handleGoLive = async (e) => {
    ...
    try {
      ...
      setStream(newStream);
      liveCtx?.startLiveSession(newStream, localStream);
      setStep('live');
  ...
  // Minimize — show mini player, navigate away
  const handleMinimize = () => {
    liveCtx?.minimize();
    liveCtx?.startLiveSession(stream, localStream);
    navigate('/');
  };
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="relative flex-1 bg-black flex flex-col min-w-0">
            ...
            <div className="flex-1 relative min-h-0">
              {/* Host self-view — always shown, mirrored for front cam */}
              {!camOn ? (
            ...
            <div className="relative p-3 bg-dark-900/90 border-t border-white/5 z-20 flex items-center gap-3">
              <HostMediaPanel
                streamId={stream._id}
                localStream={localStream}
                setLocalStream={(s) => { setLocalStream(s); if (liveCtx) liveCtx.localStream.current = s; }}
              ...
              {isMobileDevice && (
                <button onClick={flipCamera}
                  className="p-2.5 bg-dark-700 hover:bg-dark-600 text-white rounded-xl transition-all flex items-center gap-1.5 text-xs font-medium shrink-0">
          ...
          <div className="relative bg-black aspect-video rounded-t-2xl overflow-hidden">
            {camError
              ? <div className="absolute inset-0 flex flex-col items-center justify-center text-white/50 p-6 text-center">
                  ...
                  <button onClick={() => startCamera()} className="btn-ghost text-sm mt-3 py-1.5">Try Again</button>
                </div>
              : <video
            ...
            {/* Flip button on preview */}
            {isMobileDevice && !camError && (
              <button onClick={flipCamera}
        ...
        <div className="glass-card p-6">
          <form onSubmit={handleGoLive} className="space-y-4">
            ...
            <button type="submit" disabled={loading}
              className="w-full py-3 bg-gradient-to-r from-red-500 to-red-600 text-white font-bold rounded-xl hover:from-red-400 hover:to-red-500 transition-all flex items-center justify-center gap-2 shadow-lg shadow-red-500/30">
              {loading
