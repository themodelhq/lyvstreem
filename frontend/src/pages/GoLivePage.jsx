...
import VirtualBackgroundVideo from '../components/VirtualBackgroundVideo';
...
export default function GoLivePage() {
  ...
  // Solo-mode WebRTC broadcasting is now handled by <HostStreamBroadcaster />
  // mounted globally in App.jsx. That component owns the peer connections to
  // every viewer for the whole live session, so they survive minimising the
  // stream / navigating away from this page.
  ...
  const updateLiveCover = async (e) => {
    ...
    try {
      const dataUrl = await readCoverFile(file);
      // Cover image can be a few MB of base64 — give it 60 s.
      const res = await api.post(`/streams/${stream._id}/cover`, { thumbnail: dataUrl }, { timeout: 60000 });
  ...
  const handleGoLive = async (e) => {
    ...
    try {
      ...
      // 60 s timeout — cold-starting the backend can take 30–60 s on free
      // hosting tiers, so a stricter timeout would consistently fail the
      // first go-live of the session.
  ...
  const handleEndStream = async () => {
    ...
    try {
      ...
      // 404 → stream was already cleaned up server-side (typically by the
      // abandoned-stream sweep after a host disconnect). That's the desired
      // end-state, so don't surface it as an error.
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          <div className="relative flex-1 bg-black flex flex-col min-w-0">
            ...
            <div className="flex-1 relative min-h-0" style={getBgStyle()}>
              {/* Raw camera feed — kept mounted so videoRef binding never breaks.
                  Hidden when a virtual background is active (the canvas takes over)
                  or when the camera is off. */}
              <video
                ref={videoRef}
                autoPlay muted playsInline
                className={`absolute inset-0 w-full h-full object-cover transition-opacity duration-300
                  ${facingMode === 'user' ? 'scale-x-[-1]' : ''}
                  ${camOn && !streamBackground ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
              />

              {/* Virtual background — runs MediaPipe selfie segmentation on
                  the camera and composites the chosen background BEHIND the
                  host. When the host walks out of frame the bg covers the
                  whole canvas naturally. */}
              {streamBackground && camOn && cameraReady && localStreamRef.current && (
              ...
              {/* Camera off — show the chosen bg full-screen, or the dark
                  fallback if none is set. */}
              {!camOn && streamBackground && (
                <div className="absolute inset-0 z-[2]" style={getBgStyle()}>
                  <div className="absolute inset-0 bg-black/30 flex items-center justify-center">
                    <div className="text-center">
                      <FiVideoOff className="text-white/50 text-5xl mx-auto mb-2 drop-shadow-lg" />
                      <p className="text-white/70 text-sm drop-shadow">Camera off</p>
                    </div>
                  </div>
                </div>
              )}
              {!camOn && !streamBackground && (
                <div className="absolute inset-0 flex items-center justify-center bg-dark-900/80 z-[2]">
                  <div className="text-center">
              ...
              {/* Top bar — viewer count + minimize only.
                  LIVE badge and stream timer live on the host dashboard now. */}
          ...
          <div className="w-64 xl:w-72 bg-dark-900 border-l border-white/5 flex flex-col overflow-hidden">
            {/* Live status row — moved off the video so it doesn't sit on top
                of the host's face. Only the host (this dashboard) sees this. */}
            <div className="px-4 pt-4 pb-3 border-b border-white/10 flex items-center gap-2">
              <span className="live-badge">● LIVE</span>
              <span className="bg-dark-700 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 font-mono">
                <FiClock className="text-[10px]" />
                <StreamClock startedAt={stream.startedAt} />
              </span>
            </div>
