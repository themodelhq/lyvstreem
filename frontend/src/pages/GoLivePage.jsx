...
export default function GoLivePage() {
  ...
  // ── Solo-mode WebRTC: serve our camera feed to each viewer that asks for it
  // Peer-to-peer mesh — fine for low viewer counts. The host opens a fresh
  // RTCPeerConnection per viewer, attaches the local MediaStream, and pumps
  // an offer through the socket relay. ICE candidates and the answer come
  // back via the same relay.
  useEffect(() => {
    if (!socket || step !== 'live' || !stream?._id || !cameraReady) return;
    ...
    const closePeer = (viewerSocketId) => {
      ...
      if (pc) { try { pc.close(); } catch (_) {} peers.delete(viewerSocketId); }
    };
    ...
    const createPeer = async (viewerSocketId) => {
      ...
      try {
        ...
        socket.emit('solo_offer', { targetSocketId: viewerSocketId, offer, streamId: stream._id });
    ...
    const onAnswer = async ({ fromSocketId, answer }) => {
      ...
      try { await pc.setRemoteDescription(answer); } catch (_) {}
    };
  ...
  }, [socket, step, stream?._id, cameraReady]);
