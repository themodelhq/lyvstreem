...
export default function GoLivePage() {
  ...
  const attachToVideo = useCallback((stream, el) => {
    ...
    if (target.srcObject === stream) return;     // already attached
    target.srcObject = stream;
    target.play().catch(() => {});               // suppress autoplay errors
  }, []);

  // ── Core camera start — multiple fallback strategies ──────────────────────
  const startCamera = useCallback(async (facing = 'user') => {
    ...
    for (const constraints of attemptList) {
      try {
        ...
        if (liveCtxRef.current) liveCtxRef.current.localStream.current = stream;
        ...
        // Attach to video after next paint
        requestAnimationFrame(() => {
          if (mountedRef.current) attachToVideo(stream);
        });
  ...
  useEffect(() => {
    ...
    if (restoreId && liveCtxRef.current?.activeStream?._id === restoreId) {
      ...
      if (existing && existing.active) {
        ...
        setCameraReady(true);
        setCamError(null);
        requestAnimationFrame(() => { if (mountedRef.current) attachToVideo(existing); });
      } else {
    ...
    return () => {
      ...
      if (!liveCtxRef.current?.isMinimized) {
        stopCamera();
      }
  ...
  useEffect(() => {
    if (localStreamRef.current && cameraReady) {
      requestAnimationFrame(() => {
        if (mountedRef.current) attachToVideo(localStreamRef.current);
      });
    }
  ...
  const handleGoLive = async (e) => {
    ...
    try {
      ...
      setStep('live');
      // Attach camera after live view renders
      requestAnimationFrame(() => {
        requestAnimationFrame(() => {
          if (mountedRef.current) attachToVideo(localStreamRef.current);
        });
