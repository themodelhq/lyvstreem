...
io.on('connection', (socket) => {
  ...
  socket.on('solo_request_stream', async ({ streamId }) => {
    ...
    try {
      ...
      // Use socket.io's room membership directly — it's atomically updated
      // when sockets join/leave/disconnect, unlike our hand-maintained
      // userSockets map which can briefly lie during a fast reconnect and
      // cause the viewer's status to oscillate between "Requesting…" and
      // "Host is offline".
      const hostRoom    = `user_${s.streamerId}`;
      ...
      io.to(hostRoom).emit('solo_viewer_wants_stream', { viewerSocketId: socket.id, streamId });
    } catch (_) {}
  });
  ...
  socket.on('disconnect', async () => {
    ...
    // Only forget the user→socket mapping if it still points to THIS socket.
    // Without this guard, a fast reconnect (tab A → tab B same user → A
    // disconnects) deletes tab B's still-valid entry and the next viewer
    // request gets a spurious "host_offline" until the next reconnect.
    if (ud && userSockets.get(ud.userId) === socket.id) userSockets.delete(ud.userId);
    ...
    for (const [streamId, viewers] of streamViewers.entries()) {
      if (viewers.has(socket.id)) {
        ...
        // that was serving this viewer. Emit to the host's user room — if
        // the room is empty (host genuinely offline) socket.io makes the
        // emit a no-op so there's no need to pre-check `userSockets`.
        try {
          const s = await Stream.findById(streamId).select('streamerId');
          if (s) io.to(`user_${s.streamerId}`).emit('solo_viewer_left', { viewerSocketId: socket.id });
        } catch (_) {}
        if (ud) {
          const room = await RoomSession.findOne({ streamId, isActive: true });
          if (room) {
