...
mongoose.connect(process.env.MONGODB_URI || 'mongodb://localhost:27017/lyvstreem')
  .then(() => {
    ...
    // Clean up abandoned streams on startup
...
async function cleanupAbandonedStreams() {
  try {
    ...
    for (const s of abandoned) {
      ...
      await Stream.findByIdAndUpdate(s._id, { isLive: false, endedAt: new Date(), duration: dur });
      await User.findByIdAndUpdate(s.streamerId, { $inc: { totalStreamMinutes: Math.floor(dur / 60), monthlyStreamMinutes: Math.floor(dur / 60) } });
      io.to(`stream_${s._id}`).emit('stream_ended', { streamId: s._id, reason: 'abandoned' });
      count++;
    }
    if (count > 0) console.log(`🧹 Cleaned up ${count} abandoned streams`);
  } catch (e) { console.error('Cleanup error:', e); }
}

// Run cleanup every 5 minutes
setInterval(cleanupAbandonedStreams, 5 * 60 * 1000);
