...
io.on('connection', (socket) => {
  ...
  socket.on('solo_request_stream', async ({ streamId }) => {
    ...
    try {
      ...
      const hostSocketId = userSockets.get(s.streamerId.toString());
      if (!hostSocketId) return socket.emit('solo_host_offline', { streamId });
      io.to(hostSocketId).emit('solo_viewer_wants_stream', { viewerSocketId: socket.id, streamId });
    } catch (_) {}
  });
  ...
  socket.on('disconnect', async () => {
    ...
    for (const [streamId, viewers] of streamViewers.entries()) {
      if (viewers.has(socket.id)) {
        ...
        // that was serving this viewer.
        try {
          const s = await Stream.findById(streamId).select('streamerId');
          const hostSocketId = s && userSockets.get(s.streamerId.toString());
          if (hostSocketId) io.to(hostSocketId).emit('solo_viewer_left', { viewerSocketId: socket.id });
        } catch (_) {}
        if (ud) {
          const room = await RoomSession.findOne({ streamId, isActive: true });
          if (room) {
