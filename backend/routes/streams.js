const router  = require('express').Router();
const { v4: uuidv4 } = require('uuid');
const Stream  = require('../models/Stream');
const User    = require('../models/User');
const { authenticateToken, optionalAuth } = require('../middleware/auth');

// ── Live streams list ─────────────────────────────────────────────────────────
router.get('/live', async (req, res) => {
  try {
    const { category, limit = 24, page = 1 } = req.query;
    const query = { isLive: true };
    if (category && category !== 'All') query.category = category;

    const streams = await Stream.find(query)
      .populate('streamerId', 'username displayName avatar isVerified')
      .sort({ viewerCount: -1, startedAt: -1 })
      .limit(parseInt(limit))
      .skip((parseInt(page) - 1) * parseInt(limit));

    const total = await Stream.countDocuments(query);
    res.json({ streams, total, page: parseInt(page) });
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch streams' });
  }
});

// ── Featured ──────────────────────────────────────────────────────────────────
router.get('/featured', async (req, res) => {
  try {
    const streams = await Stream.find({ isLive: true })
      .populate('streamerId', 'username displayName avatar isVerified')
      .sort({ viewerCount: -1 })
      .limit(6);
    res.json(streams);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch featured' });
  }
});

// ── My streams ────────────────────────────────────────────────────────────────
router.get('/my/streams', authenticateToken, async (req, res) => {
  try {
    const streams = await Stream.find({ streamerId: req.user.userId })
      .sort({ createdAt: -1 })
      .limit(20);
    res.json(streams);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch streams' });
  }
});

// ── Categories metadata ───────────────────────────────────────────────────────
router.get('/meta/categories', async (req, res) => {
  res.json([
    { id: 'entertainment', name: 'Entertainment', emoji: '🎭' },
    { id: 'gaming',        name: 'Gaming',        emoji: '🎮' },
    { id: 'music',         name: 'Music',         emoji: '🎵' },
    { id: 'talk-show',     name: 'Talk Show',     emoji: '🎤' },
    { id: 'beauty',        name: 'Beauty',        emoji: '💄' },
    { id: 'fitness',       name: 'Fitness',       emoji: '💪' },
    { id: 'cooking',       name: 'Cooking',       emoji: '👨‍🍳' },
    { id: 'travel',        name: 'Travel',        emoji: '✈️'  },
    { id: 'education',     name: 'Education',     emoji: '📚' },
    { id: 'sports',        name: 'Sports',        emoji: '⚽' },
    { id: 'comedy',        name: 'Comedy',        emoji: '😂' },
    { id: 'fashion',       name: 'Fashion',       emoji: '👗' },
  ]);
});

// ── Clear all stale/ended streams for host profile ────────────────────────────
router.delete('/clear-stale', authenticateToken, async (req, res) => {
  try {
    // Force-end any still-marked-live streams, then delete all ended ones
    const live = await Stream.find({ streamerId: req.user.userId, isLive: true });
    for (const s of live) {
      const dur = s.startedAt ? Math.floor((Date.now() - new Date(s.startedAt)) / 1000) : 0;
      await Stream.findByIdAndUpdate(s._id, {
        isLive: false, endedAt: new Date(), duration: dur, viewerCount: 0,
      });
      req.app.get('io')?.to(`stream_${s._id}`).emit('stream_ended', { streamId: s._id, reason: 'force_ended' });
    }
    // Remove ended sessions from profile entirely
    const removed = await Stream.deleteMany({ streamerId: req.user.userId, isLive: false });
    res.json({ success: true, cleared: live.length, deleted: removed.deletedCount || 0 });
  } catch (err) {
    console.error('Clear stale error:', err);
    res.status(500).json({ error: 'Failed to clear streams' });
  }
});

// ── Next recommended stream ──────────────────────────────────────────────────
// Used when a viewer's current stream ends — auto-navigate them to the best
// candidate. Resolution order:
//   1. A live stream from someone the viewer FOLLOWS  ("friends first")
//   2. A live stream in the SAME CATEGORY as the one that just ended
//   3. The most-watched live stream overall
//   4. null (no other live streams)
router.get('/next-recommended', optionalAuth, async (req, res) => {
  try {
    const { excludeStreamId, currentCategory } = req.query;
    const userId = req.user?.userId;

    const baseFilters = [{ isLive: true }];
    if (excludeStreamId) baseFilters.push({ _id: { $ne: excludeStreamId } });
    if (userId)          baseFilters.push({ streamerId: { $ne: userId } });

    const populate = (q) => q
      .populate('streamerId', 'username displayName avatar isVerified')
      .sort({ viewerCount: -1, startedAt: -1 });

    // 1. Friends (followed users) currently live
    if (userId) {
      const me = await User.findById(userId).select('following');
      if (me?.following?.length) {
        const friendStream = await populate(Stream.findOne({
          $and: [...baseFilters, { streamerId: { $in: me.following } }],
        }));
        if (friendStream) return res.json({ stream: friendStream, source: 'friend' });
      }
    }

    // 2. Same category
    if (currentCategory) {
      const sameCat = await populate(Stream.findOne({
        $and: [...baseFilters, { category: currentCategory }],
      }));
      if (sameCat) return res.json({ stream: sameCat, source: 'category' });
    }

    // 3. Most-watched live stream
    const popular = await populate(Stream.findOne({ $and: baseFilters }));
    return res.json({ stream: popular || null, source: popular ? 'popular' : 'none' });
  } catch (err) {
    console.error('next-recommended error:', err);
    res.status(500).json({ error: 'Failed to fetch recommendation' });
  }
});

// ── Single stream ─────────────────────────────────────────────────────────────
router.get('/:streamId', optionalAuth, async (req, res) => {
  try {
    const stream = await Stream.findById(req.params.streamId)
      .populate('streamerId', 'username displayName avatar isVerified bio totalFollowers totalLikes');
    if (!stream) return res.status(404).json({ error: 'Stream not found' });
    res.json(stream);
  } catch (err) {
    res.status(500).json({ error: 'Failed to fetch stream' });
  }
});

// Validate a cover image — accept http(s) URL or a small data URL only
const MAX_COVER_BYTES = 4 * 1024 * 1024;  // ~4 MB
function sanitiseCover(value) {
  if (typeof value !== 'string' || !value) return '';
  if (value.startsWith('http://') || value.startsWith('https://')) return value.slice(0, 2048);
  if (value.startsWith('data:image/')) {
    if (value.length > MAX_COVER_BYTES * 1.4) return null; // too big
    return value;
  }
  return null;
}

// ── Go Live (atomic: end existing + create + start in one call) ───────────────
router.post('/go-live', authenticateToken, async (req, res) => {
  try {
    const { title, description, category, tags, thumbnail } = req.body;
    if (!title || !title.trim()) {
      return res.status(400).json({ error: 'Title required' });
    }
    const cover = sanitiseCover(thumbnail);
    if (cover === null) return res.status(400).json({ error: 'Invalid cover image' });

    const hostId = req.user.userId;

    // 1. End ALL existing live streams for this host (prevents duplicates)
    //    AND remove them so they don't linger as stale entries on the profile.
    const existingLive = await Stream.find({ streamerId: hostId, isLive: true });
    for (const old of existingLive) {
      req.app.get('io')?.to(`stream_${old._id}`).emit('stream_ended', {
        streamId: old._id,
        reason: 'replaced',
      });
    }
    if (existingLive.length) {
      await Stream.deleteMany({ _id: { $in: existingLive.map(s => s._id) } });
    }

    // 2. Mark user as streamer
    await User.findByIdAndUpdate(hostId, { isStreamer: true });

    // 3. Create & immediately start the new stream (atomic)
    const stream = await Stream.create({
      streamerId:    hostId,
      title:         title.trim(),
      description:   description || '',
      category:      category || 'Entertainment',
      tags:          Array.isArray(tags) ? tags : [],
      thumbnail:     cover || '',
      streamKey:     uuidv4(),
      isLive:        true,
      startedAt:     new Date(),
      lastHeartbeat: new Date(),
      viewerCount:   0,
    });

    const populated = await Stream.findById(stream._id)
      .populate('streamerId', 'username displayName avatar isVerified');

    // 4. Broadcast to all connected clients
    req.app.get('io')?.emit('stream_started', { stream: populated });

    res.status(201).json(populated);
  } catch (err) {
    console.error('Go-live error:', err);
    res.status(500).json({ error: 'Failed to go live. Please try again.' });
  }
});

// ── End stream ────────────────────────────────────────────────────────────────
router.post('/:streamId/end', authenticateToken, async (req, res) => {
  try {
    // Find stream - allow ending if host OR if the stream exists (for cleanup)
    let stream = await Stream.findOne({
      _id: req.params.streamId,
      streamerId: req.user.userId,
    });
    // Fallback: try finding by ID only (handles edge cases)
    if (!stream) {
      stream = await Stream.findById(req.params.streamId);
      if (!stream) return res.status(404).json({ error: 'Stream not found' });
      if (stream.streamerId.toString() !== req.user.userId) {
        return res.status(403).json({ error: 'Not your stream' });
      }
    }

    const duration = stream.startedAt
      ? Math.floor((new Date() - new Date(stream.startedAt)) / 1000)
      : 0;

    // Build session summary BEFORE deletion so the host still sees stats
    const summary = {
      title:        stream.title,
      category:     stream.category,
      duration:     duration,
      durationMin:  Math.floor(duration / 60),
      peakViewers:  stream.peakViewers || 0,
      totalGifts:   stream.totalGiftsValue || 0,
      startedAt:    stream.startedAt,
      endedAt:      new Date(),
    };

    // Credit streaming minutes to host
    const durationMin = Math.floor(duration / 60);
    if (durationMin > 0) {
      const host = await User.findById(req.user.userId);
      if (host) {
        const now = new Date(), lr = new Date(host.lastMonthlyReset || 0);
        if (now.getMonth() !== lr.getMonth() || now.getFullYear() !== lr.getFullYear()) {
          host.monthlyStreamMinutes = 0;
          host.lastMonthlyReset     = now;
        }
        host.monthlyStreamMinutes += durationMin;
        host.totalStreamMinutes   += durationMin;
        await host.save();
      }
    }

    req.app.get('io')?.to(`stream_${stream._id}`).emit('stream_ended', { streamId: stream._id });

    // Remove the stream document so it doesn't sit on the profile as stale
    await Stream.findByIdAndDelete(stream._id);

    res.json({ success: true, summary });
  } catch (err) {
    console.error('End stream error:', err);
    res.status(500).json({ error: 'Failed to end stream' });
  }
});

// ── Force-end all live streams for this host (safety endpoint) ────────────────
router.post('/end-all', authenticateToken, async (req, res) => {
  try {
    const hostId = req.user.userId;
    const live = await Stream.find({ streamerId: hostId, isLive: true });
    for (const s of live) {
      req.app.get('io')?.to(`stream_${s._id}`).emit('stream_ended', { streamId: s._id });
    }
    if (live.length) {
      await Stream.deleteMany({ _id: { $in: live.map(s => s._id) } });
    }
    res.json({ success: true, ended: live.length });
  } catch (err) {
    res.status(500).json({ error: 'Failed' });
  }
});

// ── Keep legacy /create + /start routes working (backward compat) ────────────
// These now delegate to the same safe atomic logic
router.post('/create', authenticateToken, async (req, res) => {
  req.body = req.body || {};
  const { title, description, category, tags } = req.body;
  if (!title || !title.trim()) return res.status(400).json({ error: 'Title required' });
  try {
    const hostId = req.user.userId;
    // End + remove any existing live streams to prevent ghosts
    const existing = await Stream.find({ streamerId: hostId, isLive: true });
    for (const old of existing) {
      req.app.get('io')?.to(`stream_${old._id}`).emit('stream_ended', { streamId: old._id, reason: 'replaced' });
    }
    if (existing.length) {
      await Stream.deleteMany({ _id: { $in: existing.map(s => s._id) } });
    }
    await User.findByIdAndUpdate(hostId, { isStreamer: true });
    const stream = await Stream.create({
      streamerId: hostId,
      title: title.trim(),
      description: description || '',
      category: category || 'Entertainment',
      tags: Array.isArray(tags) ? tags : [],
      streamKey: uuidv4(),
    });
    const populated = await Stream.findById(stream._id).populate('streamerId', 'username displayName avatar');
    res.status(201).json(populated);
  } catch (err) {
    console.error('Create error:', err);
    res.status(500).json({ error: 'Stream creation failed' });
  }
});

router.post('/:streamId/start', authenticateToken, async (req, res) => {
  try {
    const stream = await Stream.findOne({ _id: req.params.streamId, streamerId: req.user.userId });
    if (!stream) return res.status(404).json({ error: 'Stream not found' });
    stream.isLive        = true;
    stream.startedAt     = new Date();
    stream.lastHeartbeat = new Date();
    stream.viewerCount   = 0;
    await stream.save();
    req.app.get('io')?.emit('stream_started', { streamId: stream._id, title: stream.title });
    res.json(stream);
  } catch (err) {
    res.status(500).json({ error: 'Failed to start stream' });
  }
});

// ── Update cover photo for a stream session ──────────────────────────────────
router.post('/:streamId/cover', authenticateToken, async (req, res) => {
  try {
    const cover = sanitiseCover(req.body?.thumbnail);
    if (cover === null) return res.status(400).json({ error: 'Invalid cover image' });

    const stream = await Stream.findOneAndUpdate(
      { _id: req.params.streamId, streamerId: req.user.userId },
      { thumbnail: cover || '' },
      { new: true }
    );
    if (!stream) return res.status(404).json({ error: 'Stream not found' });

    // Tell viewers a new cover is up
    req.app.get('io')?.to(`stream_${stream._id}`).emit('stream_cover_changed', {
      streamId: stream._id,
      thumbnail: stream.thumbnail,
    });

    res.json({ success: true, thumbnail: stream.thumbnail });
  } catch (err) {
    console.error('Cover update error:', err);
    res.status(500).json({ error: 'Failed to update cover' });
  }
});

module.exports = router;
