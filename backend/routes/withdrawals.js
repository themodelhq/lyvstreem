...
router.post('/request', authenticateToken, async (req, res) => {
  try {
    ...
    if (user.earningCoins < minCoins) {
      return res.status(400).json({ error: `Minimum ${minCoins.toLocaleString()} coins required` });
    }
    if (coinsToWithdraw > user.earningCoins) {
    ...
    user.earningCoins -= coinsToWithdraw;
    user.totalWithdrawnCoins += coinsToWithdraw;
    ...
    res.json({
      ...
      remainingCoins: user.earningCoins,
    });
  } catch (err) {
    console.error(err);
