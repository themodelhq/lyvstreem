...
// Contribute coins to PK battle (from gift sending on PK stream)
router.post('/:battleId/contribute', authenticateToken, async (req, res) => {
  try {
    const { side, coins } = req.body; // side: 'A' or 'B'
    const battle = await PKBattle.findById(req.params.battleId);
    if (!battle || battle.status !== 'active') return res.status(400).json({ error: 'Battle not active' });

    const field = side === 'A' ? 'coinsA' : 'coinsB';
    battle[field] += coins;
    await battle.save();

    const payload = { battleId: battle._id, coinsA: battle.coinsA, coinsB: battle.coinsB };
    req.app.get('io').to(`stream_${battle.streamA}`).emit('pk_update', payload);
    req.app.get('io').to(`stream_${battle.streamB}`).emit('pk_update', payload);
